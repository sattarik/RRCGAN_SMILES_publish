...
from tensorflow.keras.layers import (Input, Dropout, LSTM, Reshape, LeakyReLU,
                          ...
                          Conv2D, MaxPooling2D, UpSampling2D, Lambda, Conv2DTranspose)
...
R1 = UpSampling2D(size = config.upFilters[0])(R1)
R20 = UpSampling2D(size = config.upFilters[0])(R2)
for i in range(3):
    ...
    C1 = Conv2D(filters = config.genFilters[i], 
               ...
               padding = 'same')(R1)
    """
    C1 = Conv2DTranspose(filters=config.genFilters[i], kernel_size=3, strides = (1, 1), padding='same')(R1)
